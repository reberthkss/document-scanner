...
public class BitmapOpenCV {
    ...
    private DetectionMethod detectionMethod = DetectionMethod.SIDES;
    ...
    public List<PointF> detectEdges() {
        ...
        if (this.originalBitmap != null) {
            ...
            // 1 - Trying with original image
            List<PointF> result = detectEdgesFromMat(image);
            ...
            // 2 - Trying inverting image colors
            image = new Mat();
            bitmapToMat(this.originalBitmap, image);

            Core.bitwise_not(image, image);

            result = detectEdgesFromMat(image);
            image.release();
    ...
    private List<PointF> detectEdgesFromMat(Mat image) {
        ...
        switch (detectionMethod) {
            case SIDES:
                points = this.findLargestSquaresBySides(squares);
                break;
            case EDGES:
                points = this.findLargestSquaresByEdges(squares);
                break;
        }
    ...
    private Point[] findLargestSquaresBySides(List<MatOfPoint> squares) {
        int i = 0;
        for (MatOfPoint c : squares) {
            i++;
            Mat pictureOut = Mat.zeros(imageSize, CvType.CV_8U);
            Scalar s = new Scalar(200, 200, 200, 200);
            List<MatOfPoint> l = new ArrayList<>();
            l.add(c);
            Imgproc.drawContours(pictureOut, l, -1, s, 3);
            Mat lines = new Mat();
            double minSize = 60;
            Imgproc.HoughLinesP(pictureOut, lines, 1, Math.PI / 180, 50, minSize, 5.0);

            if (this.debug) {
                Log.d("DBG", "For contour " + i + ": " + lines.rows() + " lines");
                for (int z = 0; z < lines.rows(); z++) {
                    double[] lineCoords = lines.get(z, 0);
                    Imgproc.line(pictureOut, new Point(lineCoords[0], lineCoords[1]), new Point(lineCoords[2], lineCoords[3]), new Scalar(100, 100, 100, 100), 3, Imgproc.LINE_AA, 0);
                    Imgproc.line(pictureOut, new Point(lineCoords[0], lineCoords[1]), new Point(lineCoords[0], lineCoords[1]), new Scalar(100, 100, 100, 100), 8, Imgproc.LINE_AA, 0);
                    Imgproc.line(pictureOut, new Point(lineCoords[2], lineCoords[3]), new Point(lineCoords[2], lineCoords[3]), new Scalar(100, 100, 100, 100), 8, Imgproc.LINE_AA, 0);
                    double x1 = lineCoords[0];
                    double y1 = lineCoords[1];
                    double x2 = lineCoords[2];
                    double y2 = lineCoords[3];
                    Point first = new Point(0, y2 - x2 * (y2 - y1) / (x2 - x1));
                    Point second = new Point(imageSize.width, y2 + (imageSize.width - x2) * (y2 - y1) / (x2 - x1));
                    Imgproc.line(pictureOut, first, second, new Scalar(100, 100, 100, 100), 1, Imgproc.LINE_AA, 0);
                }
            }

            List<LineParameters> linesParameters = new ArrayList<>();
            for (int z = 0; z < lines.rows(); z++) {
                double[] lineCoords = lines.get(z, 0);
                Point p1 = new Point(lineCoords[0], lineCoords[1]);
                Point p2 = new Point(lineCoords[2], lineCoords[3]);
                LineParameters params = ImageCalculationUtils.twoPointsToPolar(p1, p2);
                linesParameters.add(params);
                if (this.debug) {
                    Log.d("DBG", "For countour " + i + ": Line " + z + ": " + params);
                }
            }

            List<LineParameters> similarLines = ImageCalculationUtils.filterSimilarLines(linesParameters, 30, Math.PI / 180 * 10);
            if (this.debug) {
                for (int z = 0; z < similarLines.size(); z++) {
                    Log.d("DBG", "For contour " + i + ": Similar line " + z + ": " + similarLines.get(z));
                }
            }
            ...
            if (similarLines.size() == 4) {
                ...
                List<List<LineParameters>> groupedByParallel = ImageCalculationUtils.groupByNearParallelLines(similarLines, Math.PI/180*45);
                if (groupedByParallel.size() == 2 && groupedByParallel.get(0).size() == 2 && groupedByParallel.get(1).size() == 2) {
                    // Looks like we have pairs of near parallel lines, so bigger chance is a rectangle
                    Point[] intersections = ImageCalculationUtils.getIntersectionsFromParallelLines(groupedByParallel);
                    if (this.debug) {
                        Imgproc.line(pictureOut, new Point(intersections[0].x, intersections[0].y), new Point(intersections[0].x, intersections[0].y), new Scalar(80, 80, 80, 80), 12, Imgproc.LINE_AA, 0);
                        Imgproc.line(pictureOut, new Point(intersections[1].x, intersections[1].y), new Point(intersections[1].x, intersections[1].y), new Scalar(80, 80, 80, 80), 12, Imgproc.LINE_AA, 0);
                        Imgproc.line(pictureOut, new Point(intersections[2].x, intersections[2].y), new Point(intersections[2].x, intersections[2].y), new Scalar(80, 80, 80, 80), 12, Imgproc.LINE_AA, 0);
                        Imgproc.line(pictureOut, new Point(intersections[3].x, intersections[3].y), new Point(intersections[3].x, intersections[3].y), new Scalar(80, 80, 80, 80), 12, Imgproc.LINE_AA, 0);
                        Log.d("DBG", "Intersections: " +
                            ImageCalculationUtils.printablePoint(intersections[0]) + " " +
                            ImageCalculationUtils.printablePoint(intersections[1]) + " " +
                            ImageCalculationUtils.printablePoint(intersections[2]) + " " +
                            ImageCalculationUtils.printablePoint(intersections[3]) + " ");
                    }
                    Point[] sortedPoints = sortPoints(intersections);
                    if (isPossibleRectangle(sortedPoints)) {
                        foundPoints = sortedPoints;
                    }
                }
            }

            if (this.debug) {
                this.saveMatAsPicture(pictureOut, "contour-" + i + ".png");
            }
    ...
    private Point[] findLargestSquaresByEdges(List<MatOfPoint> squares){
        for (MatOfPoint c : squares) {
    ...
    private boolean isPossibleRectangle(Point[] approxPoints) {
        // angles must be ~90° (+/-10°)
        double maxcos = getMaxCosine(0, approxPoints);

        if (!(Math.abs(maxcos) <= 10.0 * Math.PI / 180)) {
            return false;
        }
